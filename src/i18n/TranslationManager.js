const API_URLS = [
  "https://api.alowork.com/api/translate",
];
...
class TranslationManager {
  constructor() {
    ...
    this.failureCount = 0;
    this.cooldownUntil = 0; // timestamp
    this.DEBUG = false;
  }
    ...
    const translationPromises = textNodes.map(async (textNode) => {
      ...
      if (originalText.trim()) {
        try {
          ...
          if (this.DEBUG) {
            console.debug(`Failed to translate chunk`, error);
          }
        }
      }
    });
  ...
  async translateText(text, lang) {
    // Circuit breaker: if in cooldown, skip remote call
    const now = Date.now();
    ...
    // All endpoints failed → increment failure count and maybe enable cooldown
    this.failureCount += 1;
